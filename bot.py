...
# === Configuration ===
BOT_TOKEN = "7714706865:AAGZW4Fq1tidbqsR0sruJBJ3_hSpaB0a0xk"
MONGO_URI = "mongodb+srv://krystalesgande:krystalesgande@cluster0.nxry6ig.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
OWNER_IDS = {"2085275594", "1623892821"}  # Multiple owner support
AUTHORIZED_USERS = set(OWNER_IDS).union({"123456789"})  # Add other authorized user IDs here
...
async def get_data_from_mongo(user_id: str):
    docs = collection.find({"user_ids": user_id})
    return list(docs)

async def save_data_to_mongo(account: dict):
    user_ids = list(AUTHORIZED_USERS)  # Include all authorized users
    user_ids.extend(OWNER_IDS)         # Ensure all owners are included
...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        await update.message.reply_text("❌ Only owners can add Gmail accounts.")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "awaiting_gmail":
        ...
        if not is_valid_base32(clean_secret):
            await update.message.reply_text("❌ Invalid 2FA secret key. Please try again.")
            return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("totp_"):
        ...
        if index >= len(accounts):
            await query.edit_message_text("❌ Invalid account index.")
            return

        account = accounts[index]
        secret = sanitize_secret(account["secret"])
        try:
...
async def fletch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 1:
        await update.message.reply_text("❌ Please specify a user ID to share your data with. Usage: /fletch <user_id>")
        return

    target_user_id = context.args[0]
    if not is_authorized(target_user_id):
        await update.message.reply_text("❌ The target user is not authorized.")
        return
    ...
    for account in accounts:
        if target_user_id not in account["user_ids"]:
            account["user_ids"].append(target_user_id)
            collection.update_one({"_id": account["_id"]}, {"$set": {"user_ids": account["user_ids"]}})

    await update.message.reply_text(f"✅ Your data has been shared with user {target_user_id}.")
...
def main():
    ...
    app.add_handler(CommandHandler("fletch", fletch))  # Add /fletch command handler
